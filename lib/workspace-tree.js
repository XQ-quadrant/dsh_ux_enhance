...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function WorkspaceTree(props) {
			...
			// TEMP diagnostics (remove after the click investigation):
			const mountSeq = React.useRef(0);
			React.useEffect(() => {
				console.log("[ux-enhance:tree] MOUNT #" + (++mountSeq.current), "sessionId=", sessionId, "cwd=", cwd);
				return () => console.log("[ux-enhance:tree] UNMOUNT #" + mountSeq.current, "sessionId=", sessionId);
			}, []);
			React.useEffect(() => {
				console.log("[ux-enhance:tree] cwd change ->", cwd);
			}, [cwd]);
			...
			React.useEffect(() => {
				...
				if (!cwd) return;
				let alive = true;
				console.log("[ux-enhance:tree] root listDirectory", cwd);
				listDirectory(cwd)
					.then((listing) => {
						console.log("[ux-enhance:tree] root ok", cwd, "entries:", listing.entries.length);
						if (alive) setRoot({ loading: false, error: null, entries: listing.entries });
					})
					.catch((err) => {
						console.log("[ux-enhance:tree] root err", cwd, err);
			...
			const toggle = (path) => {
				...
				console.log("[ux-enhance:tree] toggle", path, "->", nowOpen ? "open" : "close");
				if (nowOpen && !loadedRef.current.has(path)) {
					loadedRef.current.add(path);
					setLoading((l) => ({ ...l, [path]: true }));
					console.log("[ux-enhance:tree] listDirectory", path);
					listDirectory(path)
						.then((listing) => {
							console.log("[ux-enhance:tree] ok", path, "entries:", listing.entries.length);
							setChildren((c) => ({ ...c, [path]: listing.entries }));
						})
						.catch((err) => {
							console.log("[ux-enhance:tree] err", path, err);
